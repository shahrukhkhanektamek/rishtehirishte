<!--  Start Header Area -->
<?php include"include/header.php"; ?>
<!-- End Header Area -->

    <div class="hom-top inner_style">

    <?php include"include/header-nav.php"; ?>


    <!-- LOGIN -->
    <section class="login_page" style="background-image:url('<?=base_url('images/login.webp')?>');">
        <div class="login py-0 mt-0">
            <div class="container">
                <div class="row">

                    <div class="inn style_login row justify-content-between align-items-center">
                        <div class="lhs col-md-5">
                            <div class="tit">
                                <h2 class="text-white mb-0">Start for <b>Your <br> Matrimonial Profile</b> Easy and fast</h2>
                            </div>
                            <div class="im">
                                <img src="images/login-couple.png" alt="">
                            </div>
                            <div class="log-bg">&nbsp;</div>
                        </div>
                        <div class="rhs col-md-5">
                            <div>
                                <div class="form-login">
                                    <form class="form_data" method="POST" action="<?=base_url(route_to('auth.user.signup-action'))?>" id="registerFormData" novalidate>
                                        <div class="row gx-3">
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Candidate name <span class="text-danger">*</span></label>
                                                    <input type="text" class="form-control" id="name" name="name" placeholder="" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Age <span class="text-danger">*</span></label>
                                                    <input type="number" class="form-control" id="mobile" name="age" placeholder="" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Email address <span class="text-danger">*</span></label>
                                                    <input type="email" class="form-control" id="email" name="email" placeholder="" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Gender <span class="text-danger">*</span></label>
                                                    <div class="InputGroup style_radio">
                                                        <input type="radio" name="gender" id="male" value="1" checked />
                                                        <label for="male">
                                                            <img src="data:image/png;base64," alt="cartoon-boy">
                                                            Male
                                                        </label>

                                                        <input type="radio" name="gender" id="female" value="2" />
                                                        <label for="female">
                                                            <img src="data:image/png;base64," alt="girl">
                                                            Female
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Mobile number <span class="text-danger">*</span></label>
                                                    <input type="number" class="form-control" id="mobile" name="phone" placeholder="" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Alt. number (Optional)</label>
                                                    <input type="number" class="form-control" id="alt_phone" name="alt_phone" placeholder="">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Date of birth <span class="text-danger">*</span></label>
                                                    <input type="date" class="form-control" id="dob" name="dob" placeholder="" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Time of birth </label>
                                                    <input type="time" class="form-control" id="timepicker1" name="time_of_birth" placeholder="">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Place of birth. (Optional)</label>
                                                    <input type="text" class="form-control" id="place_of_birth" name="place_of_birth" placeholder="">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                <label class="lb">Profile For <span class="text-danger">*</span></label>
                                                <select class="chosen-select" name="profilefor" required>
                                                    <option selected value="">Select one</option>
                                                    <option>Self</option>
                                                    <option>Son</option>
                                                    <option>Daughter</option>
                                                    <option>Brother</option>
                                                    <option>Sister</option>
                                                    <option>Other</option>
                                                </select>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Create password <span class="text-danger">*</span></label>
                                                    <input type="password" class="form-control" id="password" placeholder="Enter password" name="password" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    <label class="lb">Confirm password <span class="text-danger">*</span></label>
                                                    <input type="password" class="form-control" id="cpassword" placeholder="Enter confirm password" name="cpassword" required>
                                                </div>
                                            </div>
                                            
                                            <div class="col-md-12 col-12">
                                                <button type="submit" class="cta-dark w-100">Register Here</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                                <p class="mt-2 mb-0" style="font-size: 13px;    color: rgb(133, 136, 144);"><small>By clicking on 'Register Free', you confirm that you accept the <a href="<?=base_url('term-condition') ?>" style="font-size:.935em;">Terms of use</a> and <a href="<?=base_url('privacy-policy')?>" style="font-size:.935em;">Privacy Policy.</a></small></p>
                                <div class="form-tit">
                                    <p class="text-white fw-bold mb-0">Already a Member? <a class="fw-bold" href="login.php">Login Here</a></p>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </section>
    <!-- END -->
<script>    
    $('#timepicker').mdtimepicker({
        timeFormat: 'hh:mm:ss.000', // format of the time value (data-time attribute)
        format: 'h:mm tt',    // format of the input value
        readOnly: false,       // determines if input is readonly
        hourPadding: false,
        theme: 'purple',
        okLabel: 'Ok',
        cancelLabel: 'Cancel'
    });
</script>
    
<!--  Start Footer Area -->
<?php include"include/footer.php"; ?>
<!-- End Footer Area -->     