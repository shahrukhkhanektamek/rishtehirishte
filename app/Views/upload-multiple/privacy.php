...
<ul class="multiple-image-link ui-sortable" id="multiple-image-link<?=$file_data['position'] ?>" style="list-style: none;margin: 0;padding: 0;margin-top: -1rem;" >
	<?php	
		$images_array = array();
		$images_array = $images = @$row;


		if(!empty($images_array))
		{
			$images_array = json_decode($images_array[0]);
			if(empty(@$images_array[0])) $images_array = [''];
		...
		?>
		  	<li>
	            <div class="input-group mt-2">
	                ...
	                <input type="text" class="form-control image_path" name="title<?=$file_data['columna_name'] ?>[]" placeholder="Title" value="<?=$image_path ?>" >
	                <input type="text" class="form-control image_alt" name="value<?=$file_data['columna_name'] ?>[]" placeholder="Description" value="<?=$image_alt ?>" >	              	
