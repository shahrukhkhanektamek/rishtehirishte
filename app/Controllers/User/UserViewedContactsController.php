...
class UserViewedContactsController extends BaseController
{
     protected $arr_values = array(
        'routename'=>'user.viewed-contacts.', 
        'title'=>'Viewed Profiles', 
        'table_name'=>'users',
        'page_title'=>'Viewed Profiles',
        "folder_name"=>'user/viewed-contacts',
        "upload_path"=>'upload/',
        "page_name"=>'viewed-contacts.php',
       );
    ...
    public function load_data()
    {
        ...
        // ✅ base query
        $query = $this->db->table("user_view_contacts")
            ->where(['user_view_contacts.user_id' => $user_id])
            ->where([$table_name.'.status' => 1])
            ->join("users as users","users.id=user_view_contacts.member_id","left")
            ->join("education as education","education.id={$table_name}.highestdegree","left")
            ->join("occupation as occupation","occupation.id={$table_name}.occupation","left")
            ->join("religion as religion","religion.id={$table_name}.religion","left")
            ->join("caste as caste","caste.id={$table_name}.caste","left")
        ...
        $query->orderBy("user_view_contacts".'.id','desc');
