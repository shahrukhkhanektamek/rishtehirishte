...
var form = '';
      $(document).on("submit", ".form_data",(function(e) {
        ...
        var form_ok = 1;  
        form.find("input[required], select[required]").each(function(index, item) {
            var input = $(this).prop("required"); 
            if ($(item).val()=="")
            { 
              let alertMessageInput = $(item).parent().find('label');
              // alertMessageInput.find('span').remove();
              ...
              error_message('Mandatory '+alertMessageInput.text())
