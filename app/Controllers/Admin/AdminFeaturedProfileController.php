...
class AdminFeaturedProfileController extends BaseController
{
     protected $arr_values = array(
        'routename'=>'admin-featured-profile.', 
        'title'=>'Featured Profile', 
        'table_name'=>'featured_profile',
        'page_title'=>'Featured Profile',
        "folder_name"=>'backend/admin/featured-profiile',
        "upload_path"=>'upload/',
        "page_name"=>'featured-profiile.php',
       );
    ...
    public function load_data()
    {
        ...
        ->join("users as users","users.id={$this->arr_values['table_name']}.user_id","left")
        ...
        if (!empty($filter_search_value)) {
            if($search_by==1) $data_list->like('users.name', $filter_search_value);
            if($search_by==2) $data_list->like('users.email', $filter_search_value);
            if($search_by==3) $data_list->like('users.phone', $filter_search_value);
            if($search_by==4) $data_list->like('users.user_id', $filter_search_value);
        }
